...
class ProjectController extends Controller
{
    ...
    {
        try {
            $list = Project::select('id', 'projectName', 'brochure', 'projectImage1', 'projectImage2', 'isActive', 'status', 'city')
                ->where('isActive', 1)
                ->paginate(10);

            return response()->json(["data" => $list, "status" => 200]);
        } catch (\Exception $e) {
            return response()->json(["message" => 'Oops! Something Went Wrong.' . $e->getMessage(), "data" => null, "status" => 500]);
        }
    }
    public function getAllList(Request $request)
    {
        try {
            $list = Project::select('id', 'projectName', 'projectImage1', 'projectImage2', 'isActive', 'status', 'city')
                ->paginate(10);
